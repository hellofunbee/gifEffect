...
public class Draw extends ImageView {
	...
	private void bootUp() {
		...
		bitmap = BitmapFactory.decodeResource(getResources(), Pens.getPen_bitmap()[1]);
	...
	private void mOndraw(Canvas canvas) {
		...
		case LINE:
			...
			if (j < pointList.size()) {
				...
				if (w1.drawWhat == 2) {
					if (readin) {
						bitmap = BitmapFactory.decodeResource(getResources(), w1.bitmapStyle);
		...
		case BITMAP:
			...
			if (readin) {
				if (w.bitmapStyle == p.bitmapStyle) {
					...
					bitmap = BitmapFactory.decodeResource(getResources(), w.bitmapStyle);
					p.bitmapStyle = w.bitmapStyle;
				}
	...
	public void resumeView(int mode) {
		...
		if (mode == NOMAL_TOUCH) {
			...
			if (isgif) {
				if (app.drawPic.temp != null || allpointList.size() > 0) {
	...
	public void save(AnimatedGifEncoder e, int frameNum, final Bitmap saveBm,int threadNum) {
