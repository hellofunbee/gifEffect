...
public class PicGifDraw extends ImageView {
	...
	public void setWhat(int what) {
		...
		this.what = what;
		if (what == LINE) {
			setPointBm(Color.WHITE, app.picGif.md, null);
	...
	public void delete() {
		if (!readin) {
			...
			clear(null);
			mbm = null;
			app.picGif.bgbm = null;
			Canvas c = new Canvas(app.picGif.alterbm);
		...
		try {
			...
			for (; i < size;) {
				...
				if (i % stroke == 0 || i == size) {
					...
					if (app.gifList != null) {
						for (GifDecoder gd : app.gifList) {
							temp = gd.getFrameImage(frameNum);
